...
7. **Resource Planning**: Builds a `ResourceCopyPlan` dataclass that records core helper files, alias clones, external HDL sources, and SRAM payload metadata.  The plan is passed to all copy helpers so ordering is deterministic and logging is centralised.
8. **Resource File Management**: Copies core support files (`fifo.sv`, `trigger_counter.sv`), materialises alias copies when required, and copies user-supplied SystemVerilog sources (resolving relative paths via `repo_path()`).
...
- `ResourceCopyPlan` and `CopyAction` dataclasses that describe which resources should be materialised and how; these objects replace loose tuples and make the copy orchestration unit-testable.
...
- **Resource Files**: FIFO and trigger counter templates recorded in the copy plan
